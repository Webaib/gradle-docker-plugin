...
version 2.1.0. Please refer to the library's documentation for more information on the supported Docker's client API
...
==== List images with(-out) filters and grep 

This version of plugin return from a task DockerListImages a list of images. So you can access this list per 'taskList.images'
...
task listImagesA(type: DockerListImages) {
	...
	doLast {
		...
		println listImages.images.each {println it.id}
	}
}
